...
public:
    ...
    void writeBytesThreadSafe(const QHostAddress &addr, const uint16_t port, const char *bytes, int length);
signals:
...
private slots:
    ...
    virtual void _writeBytes(const QHostAddress &addr, const uint16_t port, const QByteArray& bytes) = 0;
private:
