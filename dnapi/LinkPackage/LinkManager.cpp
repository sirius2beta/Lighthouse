...
void LinkManager::addPrimaryUDPLinkHost(const QString &url)
{
    SharedLinkInterfacePtr link = mavlinkPrimaryUDPLink();
    if(link){

        UDPConfiguration* const config = qobject_cast<UDPConfiguration*>(link->linkConfiguration().get());
        config->addHost(url);
    }
}
...
void LinkManager::addSecondaryUDPLinkHost(const QString &url)
{
    SharedLinkInterfacePtr link = mavlinkSecondaryUDPLink();
    if(link){
